...
import (
	...
	"github.com/ebitengine/gomobile/bind"
...
func genPkg(lang string, p *types.Package, astFiles []*ast.File, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named) {
	...
	case "java":
		...
		if p == nil {
			...
			javaDir, err := packageDir("github.com/ebitengine/gomobile/bind/java")
			if err != nil {
				errorf("unable to import bind/java: %v", err)
				return
			}
	...
	case "objc":
		...
		if p == nil {
			// Copy support files
			dir, err := packageDir("github.com/ebitengine/gomobile/bind/objc")
			if err != nil {
